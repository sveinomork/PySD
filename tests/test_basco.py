...
def test_basco_connected_sdbase_fails(validity_level=ValidationLevel.NORMAL, cross_object_validation=True):
...
def test_basco_connected_sdbase_succeeds(validity_level=ValidationLevel.NORMAL, cross_object_validation=True):
    """Test BASCO connected to SD_BASE model"""
    model = SD_BASE()
    ...
    for i in range(6):
        model.add(BASCO(id=201+i, load_cases=[LoadCase(lc_type='OLC', lc_numb=201+i)]))
...
def test_basco_connected_sdbase_failure_due_to_greco(validity_level=ValidationLevel.NORMAL, cross_object_validation=True):
    ...
    basco = BASCO(
        ...
        load_cases=[
            LoadCase(lc_type='OLC', lc_numb=101, lc_fact=1.0),
            LoadCase(lc_type='ELC', lc_numb=102, lc_fact=1.5)  # ELC without GRECO should fail
        ]
    )
