...
def test_srtyp_incteracting_with_sd_base_failure():
    ...
    model.add(
        SRTYP(id=1, mp=1, ar=753.0e-6, lb="1.0D12_c150c150"), validation=False
    )  # Skip validation for setup
    srloc = SRLOC(
        id="SR1", pa="VEGG_2", st=2
    )  # References SRTYP id=2 which doesn't exist
...
def test_srtyp_incteracting_with_sd_base_success():
    ...
    model.add(
        SRLOC(id="SR1", pa="VEGG_2", st=1), validation=False
