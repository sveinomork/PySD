...
def field_required_error(field: str, statement_type: str) -> str:
    ...
    return ErrorMessageBuilder.build_message(
        "REQUIRED_FIELD", field=field, statement_type=statement_type
    )
...
def invalid_range_error(field: str, min_val: int, max_val: int) -> str:
    ...
    return ErrorMessageBuilder.build_message(
        "INVALID_RANGE", field=field, min_val=min_val, max_val=max_val
    )
...
def invalid_length_error(field: str, max_length: int) -> str:
    ...
    return ErrorMessageBuilder.build_message(
        "INVALID_LENGTH", field=field, max_length=max_length
    )


def cross_reference_error(
    statement_type: str, source_id: str, target_type: str, target_id: str
) -> str:
...
def duplicate_id_error(statement_type: str, id_value: str) -> str:
    ...
    return ErrorMessageBuilder.build_message(
        "DUPLICATE_ID", statement_type=statement_type, id_value=id_value
    )
