...
@instance_rule('SRLOC')
def validate_srloc_instance(statement: 'SRLOC', context: 'ValidationContext') -> List[ValidationIssue]:
...
def validate_srloc_container(container: 'BaseContainer[SRLOC]', context: 'ValidationContext') -> List[ValidationIssue]:
    ...
    for stmt_id in ids:
        if stmt_id in seen_ids:
            issues.append(ValidationIssue(
                severity="error",
                code="SRLOC_DUPLICATE_ID",
                message=f"Duplicate SRLOC ID '{stmt_id}' found in container",
                location=f"SRLOC.{stmt_id}",
                suggestion="Use unique IDs for each SRLOC statement"
            ))
...
@model_rule('SRLOC')
def validate_reloc_model(statement: 'SRLOC', context: 'ValidationContext') -> List[ValidationIssue]:
