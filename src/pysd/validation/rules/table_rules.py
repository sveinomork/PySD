...
@instance_rule('TABLE')
def validate_table_mode(table: 'TABLE', context: ValidationContext) -> List[ValidationIssue]:
...
@instance_rule('TABLE')
def validate_table_parameters(table: 'TABLE', context: ValidationContext) -> List[ValidationIssue]:
    ...
    for coord_name, coord_value in [('x1', table.x1), ('x2', table.x2), ('x3', table.x3)]:
        if coord_value is not None and len(coord_value) != 3:
            issues.append(ValidationIssue(
                ...
                context={'table_id': table.id, 'coordinate': coord_name, 'value': coord_value}
            ))
...
@instance_rule('TABLE')
def validate_table_mode_specific_parameters(table: 'TABLE', context: ValidationContext) -> List[ValidationIssue]:
    """Validate that mode-specific parameters are used correctly."""
    issues = []
    
    # TAB-specific parameters should only be used with TAB mode
    tab_specific = [table.el, table.se, table.rn, table.x1, table.x2, table.x3, table.enr, table.cc]
    ...
    if table.ur is None and (any(param is not None for param in ur_specific) or any(ur_specific_bool)):
        issues.append(ValidationIssue(
            severity='warning',
            code='TABLE_012',
...
@instance_rule('TABLE')
def validate_table_file_output(table: 'TABLE', context: ValidationContext) -> List[ValidationIssue]:
...
def validate_table_uniqueness(container: 'TableContainer', context: ValidationContext) -> List[ValidationIssue]:
    ...
    for table in container.items:
        ...
        config = (
            table.tab, table.ur, table.pa, table.fs, table.hs, table.ls,
            table.ilc, table.olc, table.elc, table.bas, table.pha
        )
        ...
        if config in seen_configs:
            issues.append(ValidationIssue(
                ...
                context={
                    'table_id': table.id,
                    'config': {
                        'tab': table.tab, 'ur': table.ur, 'pa': table.pa,
                        'fs': table.fs, 'hs': table.hs, 'ls': table.ls
                    }
                }
            ))
        else:
