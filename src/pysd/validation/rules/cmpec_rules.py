...
@instance_rule("CMPEC")
def validate_cmpec_id_range(
    obj: "CMPEC", context: ValidationContext
...
@instance_rule("CMPEC")
def validate_cmpec_concrete_grade(
    obj: "CMPEC", context: ValidationContext
) -> List[ValidationIssue]:
...
@instance_rule("CMPEC")
def validate_cmpec_density_range(
    obj: "CMPEC", context: ValidationContext
) -> List[ValidationIssue]:
...
def validate_cmpec_uniqueness(
    obj: "CMPEC", context: ValidationContext
) -> List[ValidationIssue]:
    """Validate CMPEC ID uniqueness in container."""
    if (
        obj.id is not None
        and context.parent_container
        and context.parent_container.contains(obj.id)
    ):
        return [
            ValidationIssue(
                severity=ValidationSeverity.ERROR.value,
...
@model_rule("CMPEC")
def validate_cmpec_part_references(
    obj: "CMPEC", context: ValidationContext
) -> List[ValidationIssue]:
