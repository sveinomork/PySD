...
@instance_rule("RMPEC")
def validate_rmpec_id_range(
    obj: "RMPEC", context: ValidationContext
...
@instance_rule("RMPEC")
def validate_rmpec_positive_values(
    obj: "RMPEC", context: ValidationContext
) -> List[ValidationIssue]:
...
@instance_rule("RMPEC")
def validate_rmpec_strength_relationship(
    obj: "RMPEC", context: ValidationContext
) -> List[ValidationIssue]:
...
@model_rule("RMPEC")
def validate_rmpec_usage_in_retyp(
    obj: "RMPEC", context: ValidationContext
) -> List[ValidationIssue]:
    ...
    if retyp_container:
        ...
        rmpec_used = any(
            retyp.mp == obj.id
            for retyp in retyp_container.items
            if hasattr(retyp, "mp")
        )
