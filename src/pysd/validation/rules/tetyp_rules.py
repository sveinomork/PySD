...
@instance_rule('TETYP')
def validate_TETYP_instance(statement: 'TETYP', context: 'ValidationContext') -> List[ValidationIssue]:
...
def validate_TETYP_container(container: 'BaseContainer[TETYP]', context: 'ValidationContext') -> List[ValidationIssue]:
...
@model_rule('TETYP')
def validate_TETYP_model(statement: 'TETYP', context: 'ValidationContext') -> List[ValidationIssue]:
    ...
    if hasattr(model, 'teloc'):
        ...
        if hasattr(model.teloc, 'items'):
            referencing_telocs = [teloc for teloc in model.teloc.items if hasattr(teloc, 'rt') and teloc.rt == statement.id]
