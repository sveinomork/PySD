...
class SD_BASE(BaseModel):
    """ ShellDesign model main entrypoint to use PySD
    ...
    all_items: List[StatementType] = Field(
        default_factory=list, exclude=True, description="Ordered list of all items"
    )
    ...
    validation_level: ValidationLevel = Field(
        ...
        exclude=True,
        description="Validation level: ValidationLevel enum",
    )
    cross_object_validation: bool = Field(
        default=True,
        exclude=True,
        description="Enable immediate cross-object validation during add()",
    )

    # ValidationManager and StatementRouter for extracted logic - initialized in model_validator
    router: Any = Field(
        default=None, exclude=True, description="Statement routing system"
    )
    ...
    def __init__(
        ...
        cross_object_validation: bool = True,
        **kwargs,
    ):
    ...
    def __getattr__(self, name: str):
        ...
        if ContainerFactory.is_valid_container(base_name):
            ...
            if (
                container_key not in self._dynamic_containers
                and base_key not in self._dynamic_containers
            ):
                ...
                for base_name_key, container in containers.items():
                    ...
                    self._dynamic_containers[f"{base_name_key}_container"] = (
                        container  # 'basco_container'
                    )
    ...
    def add(
        self,
        item: Union[StatementType, Sequence[StatementType]],
        ...
        run_cross_object_validation = self.cross_object_validation and (
            validation if validation is not None else True
        )
