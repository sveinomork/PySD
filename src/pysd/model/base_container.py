...
class BaseContainer(BaseModel, Generic[T]):
    ...
    items: List[T] = Field(
        default_factory=list, description="List of items with unique identifiers"
    )
    parent_model: Optional[Any] = Field(
        default=None,
        exclude=True,
        description="Reference to parent model for validation settings",
    )
    ...
    def _is_container_validation_enabled(self) -> bool:
        ...
        if self.parent_model and hasattr(
            self.parent_model, "container_validation_enabled"
        ):
        ...
        if self._is_container_validation_enabled():
            for item in items:
                ...
                for existing in self.items:
                    if self._normalize_id(existing.identifier) == item_id:
                        raise ValueError(
                            f"Item with identifier {item_id} already exists"
                        )
    ...
    def get_by_range(
        self, attr_name: str, min_val: Union[int, float], max_val: Union[int, float]
    ) -> List[T]:
