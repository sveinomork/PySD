...
class CMPEC(StatementBase):
    ...
    gr: Optional[str] = Field(
        None, description="Concrete grade, legal range 12-90 (integer) [MPa]"
    )
    rh: Optional[float] = Field(
        None, description="Density for lightweight concrete (1150-2150 kg/m3)"
    )

    # Single data parameters (when no concrete grade is defined)
    fck: Optional[float] = Field(
        None, description="Cylinder compression strength after 28 days [Pa]"
    )
