...
class XTFIL(StatementBase):
    ...
    fs: Optional[Union[int, Tuple[int, int]]] = Field(
        None, description="F-section range"
    )
    hs: Optional[Union[int, Tuple[int, int]]] = Field(
        None, description="H-section range"
    )
    ...
    tw: Optional[str] = Field(
        None, description="utilization ratio crack widths at tendons"
    )
    ...
    ms: Optional[str] = Field(
        None, description="Results maximum membrane stress tightness"
    )
    lf: Optional[str] = Field(None, description="location dependent load factors")
    pv: Optional[str] = Field(
        None,
        description="only peak plots of all design cases are plotted if more than one case is stored",
    )
