...
class SHEXT(StatementBase):
    ...
    efs: Optional[Tuple[int, int, int, int]] = Field(
        None, description="i,i1,i2,i3: F-section to be extrapolated from"
    )
    ehs: Optional[Tuple[int, int, int, int]] = Field(
        None, description="i,i1,i2,i3: H-section to be extrapolated from"
    )

    hs: Optional[int | Tuple[int, int]] = Field(
        None, description="H-section range (j1, j2)"
    )
    fs: Optional[int | Tuple[int, int]] = Field(
        None, description="F-section range (f1, f2)"
    )

    fact: Optional[Tuple[float, float, float]] = Field(
        None, description="f1,f2,f3: factors given in input"
    )
    dist: Optional[float] = Field(None, description="distance from last section")
    face: Optional[Literal[0, 1, 2]] = Field(
        None, description="0: Section on shell edge, 1/2: Section on solid surface"
    )
    xp: Optional[Tuple[float, float, float]] = Field(
        None,
        description="Section located on a plane surface. Defined by XP=plane point, xA=plane normal",
    )
    xc: Optional[Tuple[float, float, float]] = Field(
        None,
        description="Section located on a cylindrical surface. Defined by XC=center XA=axis direction",
    )
    xa: Optional[Tuple[float, float, float]] = Field(
        None, description="Plane normal (face=0) or axis direction (face=1,2)"
    )
    ...
    def _build_input_string(self) -> None:
        ...
        self.input = self._build_string_generic(
            ...
            special_formatting={"efs": lambda v: f"EF={','.join(str(x) for x in v)}"},
