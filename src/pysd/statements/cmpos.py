...
class COMPOS(StatementBase):
    ...
    fig: Optional[Tuple[int, int]] = Field(None, description="Define non-linear or linear material curve.")
    ...
    epu: Optional[float] = Field(None, description="Ultimate compression strain ULS/ALS [m/m]")

    csd: Optional[float] = Field(None, description="Design compression strengtherning factor")
    ...
    ftk: Optional[float] = Field(None, description="characteristic tensile strength (CRW)")
    ftn: Optional[float] = Field(None, description="In situ tensile strength (ULS/ALS/SLS/CRW)")
    
    ftm: Optional[float] = Field(None, description="In situ tensile strength [kPa]")
    ac1: Optional[float] = Field(None, description="Reduced compression strength for ULS, ALS and SLS limit states")
    ac2: Optional[float] = Field(None, description="Reduced compression strength for FLS")
    ...
    def _build_input_string(self) -> None:
        ...
        self.input = self._build_string_generic(
            field_order=['id', 'gr', 'rh', 'fig', 'fcn', 'ecn', 'epu', 'csd', 'tsp', 'tsd', 'ftk', 'ftn',
                          'ac1', 'ac2', 'mfu', 'mfa', 'mff', 'mfs',  'la', 'pa', 'fs', 'hs'],
