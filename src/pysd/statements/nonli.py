...
class NONLI(StatementBase):
    ...
    """
    spa: Optional[str] = Field(..., description="path to sestra executable when using FEA=SES")
    
    fea: Optional[Literal['SES', 'OOS']] = Field(None, description="'SES' for Sestra, 'OOS' for OOsolver")
    nsr: Optional[int] = Field(None, description="n=max number of FE analysis run per design case n=0 flags linear analysis and material properties from T-file wil be used. CMPNL data wil be ignored.")
    ndt: Optional[float] = Field(None, description="iteration stop condition, node Displacement difference threshold")
