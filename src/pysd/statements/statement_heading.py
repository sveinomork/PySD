...
class HEADING(StatementBase):
    ...
    statement: Optional[str] = Field(None, description="Statement name or section title (will be uppercase)")
    bas_id: Optional[str] = Field(None, description="BASCO group ID (automatically uses bas style)")
    description: Optional[str] = Field(None, description="Optional description (use \\n for multiple lines)")
    comment: Optional[str] = Field(None, description="Optional comment (use \\n for multiple lines)")
    line_length: int = Field(60, description="Length of the border lines")
    heading_type: Literal["default", "bas"] = Field("default", description="Heading style type")
    ...
    def validate_and_set_heading_type(self) -> 'HEADING':
        ...
        if self.statement is not None and self.bas_id is not None:
            raise ValueError("Cannot specify both 'statement' and 'bas_id'. Use one or the other.")
    ...
    def _get_emoji(self) -> str:
        ...
        if any(word in statement_lower for word in ["load", "force", "basco", "greco"]):
            return "⚡"
        elif any(word in statement_lower for word in ["material", "concrete", "steel", "cmpec", "rmpec"]):
            return "🏗️"
        elif any(word in statement_lower for word in ["geometry", "section", "desec", "shaxe"]):
            return "📐"
        elif any(word in statement_lower for word in ["analysis", "result", "table", "xtfil"]):
            return "📊"
        elif any(word in statement_lower for word in ["reinforcement", "retyp", "reloc"]):
            return "🔩"
