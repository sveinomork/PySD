...
def create_basic_model_components(sd_model: SD_BASE) -> None:
    ...
    """
    # Add FILST entry
    filst_entry = FILST(name="aquapod", vers="1.0", date="14.aug-2025", resp="som")
...
def create_design_sections(sd_model: SD_BASE) -> None:
    ...
    for counter, set_number in enumerate(vegg):
        sd_model.add(
            SHSEC(
                pa=f"VEGG_{set_number}",
                elset=set_number,
                hs=(1 + 4 * counter, 4 + 4 * counter),
            )
        )
    # veg_intern_s_e
    ...
    sd_model.add(
        SHAXE(pa="VEGG_4", hs=(1, 4), x1=(0, 1, 0), x2=(0, 0, 1), x3=(1, 0, 0))
    )
    sd_model.add(_shaxe_base_point("VEGG_5", p7, p3, p11))
    sd_model.add(
        SHAXE(pa="VEGG_6", hs=(1, 4), x1=(1, 0, 0), x2=(0, 0, 1), x3=(0, 1, 0))
    )
    # 7
...
def create_load_components(sd_model: SD_BASE) -> None:
    ...
    load_cases = [
        LoadCase(lc_type="ELC", lc_numb=i + 101, lc_fact=1.5) for i in range(6)
    ]

    load_cases2 = [
        LoadCase(lc_type="ELC", lc_numb=i + 101, lc_fact=1.8) for i in range(6)
    ]
